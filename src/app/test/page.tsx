...
export default function TestHome() {
  ...
  // --- Signup modal logic (test mode: pretend user is not signed in) ---
  const [showSignupModal, setShowSignupModal] = useState(false);
  const slideClickCountRef = useRef<number>(0);
  const nextModalThreshold = useRef<number>(5); // First modal after 5 clicks
  const modalPending = useRef<boolean>(false); // Flag: show modal on next search
  const [testClickDisplay, setTestClickDisplay] = useState(0); // For test indicator reactivity
  ...
  const handleResultClick = async (result: SearchResult, position: number) => {
    ...
    // Signup modal: track slide clicks (test mode — always acts as anonymous)
    slideClickCountRef.current++;
    setTestClickDisplay(slideClickCountRef.current);
    if (slideClickCountRef.current >= nextModalThreshold.current) {
      modalPending.current = true;
      // Set next threshold: after first (5), every 10 thereafter (15, 25, 35...)
      nextModalThreshold.current = slideClickCountRef.current + 10;
    }
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    // Signup modal: show 4s after search so typing isn't interrupted
  ...
  return (
    <main className="flex min-h-screen">
      ...
      {/* Test mode indicator */}
      <div style={{
        position: 'fixed', bottom: '0.5rem', left: '0.5rem', zIndex: 10000,
        background: '#fef3c7', border: '1px solid #fde68a', borderRadius: '0.375rem',
        padding: '0.3rem 0.6rem', fontSize: '0.65rem', color: '#92400e',
      }}>
        TEST MODE — Clicks: {testClickDisplay} | Next modal at: {nextModalThreshold.current}
      </div>
    </main>
  );
}
