...
export default function Home() {
  ...
  const slideClickCountRef2 = useRef<number>(0);
  const nextModalThreshold = useRef<number>(5);
  const modalPending = useRef<boolean>(false);

  // User stats bar
  const [userStats, setUserStats] = useState<{
    slides: { lifetime: number; month: number; week: number; today: number };
    favorites: number;
    rank: { lifetime: number; month: number; week: number; today: number };
    percentile: number;
    totalUsers: number;
  } | null>(null);
  ...
  const handleResultClick = async (result: SearchResult, position: number) => {
    ...
    // Signup modal: track slide clicks for anonymous users only
    if (!session?.user) {
      slideClickCountRef2.current++;
      if (slideClickCountRef2.current >= nextModalThreshold.current) {
        modalPending.current = true;
        nextModalThreshold.current = slideClickCountRef2.current + 10;
      }
    }
  ...
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    // Signup modal: show 4s after search for anonymous users
    if (modalPending.current && !session?.user) {
      modalPending.current = false;
      ...
      <div className="flex-1 flex flex-col items-center p-3 sm:p-4 pt-4 sm:pt-8 lg:pt-8">
        <div className="w-full max-w-5xl">
          ...
          <div className="relative mb-4 sm:mb-6 px-2">
            {/* User Stats - Positioned top left */}
            {session?.user && userStats && (
              <div className="absolute top-0 left-2 hidden sm:block" style={{ fontSize: '0.65rem', lineHeight: 1.8 }}>
                <div className="text-gray-400 dark:text-gray-500">Slides 24h: <span className="text-gray-600 dark:text-gray-300">{userStats.slides.today}</span></div>
                <div className="text-gray-400 dark:text-gray-500">Slides lifetime: <span className="text-gray-600 dark:text-gray-300">{userStats.slides.lifetime}</span>
                  {(userStats.rank.lifetime <= 10 || session?.user?.email === 'fleshbits@gmail.com') && (
                    <span style={{ color: '#991b1b' }}> (Global Rank #{userStats.rank.lifetime})</span>
                  )}
                </div>
                <div className="text-gray-400 dark:text-gray-500">Favorites: <span className="text-gray-600 dark:text-gray-300">{userStats.favorites}</span></div>
              </div>
            )}
